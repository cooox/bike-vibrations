...
const int vibsens_high = 6; //PIN empfindlicher Sensor 
const int vibsens_low = 5;  //PIN weniger empfindlicher Sensor
...
void setup() {
  ...
  pinMode(vibsens_high, INPUT_PULLUP);
  pinMode(vibsens_low, INPUT_PULLUP);
...
void loop() {
 ...
 acc_Z =event.acceleration.x;
  ...
  if (digitalRead(vibsens_high) == LOW){
    Serial.println("schwache Vibration");
  ...
  if (digitalRead(vibsens_low) == LOW){
